...
class SoundTestMenu extends MusicBeatState
{
	...
	function doTheThing(first:Int, second:Int) 
	{
		...
		else if (first == 41 && second == 1) 
		{
		...
		else if (first == 23 && second == 23) 
			{
				...
				new FlxTimer().start(2.1, function(tmr:FlxTimer)
				{
                    LoadingState.loadAndSwitchState(new VideoState('assets/videos/Keel.webm', new SoundTestMenu()));
				});
			}
		else if (first == 12 && second == 34) 
			{
				...
				new FlxTimer().start(2.1, function(tmr:FlxTimer)
				{
					LoadingState.loadAndSwitchState(new VideoState('assets/videos/Milky.webm', new SoundTestMenu()));
				});
			}
	...
	override public function update(elapsed:Float)
		{
			if (FlxG.keys.justPressed.RIGHT || FlxG.keys.justPressed.LEFT || FlxG.keys.justPressed.A || FlxG.keys.justPressed.D || controls.LEFT_P || controls.RIGHT_P) if (woahmanstopspammin) funnymonke = !funnymonke;

			if (FlxG.keys.justPressed.DOWN || FlxG.keys.justPressed.S || controls.DOWN_P) if (woahmanstopspammin) changeNumber(1);

			if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.W || controls.UP_P) if (woahmanstopspammin) changeNumber(-1);
