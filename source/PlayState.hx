...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if (curSong != SONG.song)
		{
			...
			else if (SONG.song.toLowerCase() == 'triple-trouble' || isRing)
			{
			...
			{
				dad = new Character(0, 0, 'fleetway-extras');
				dad.alpha = 0.01;
				add(dad);
				remove(dad);

				dad = new Character(0, 0, 'fleetway-extras2');
				dad.alpha = 0.01;
				add(dad);
				remove(dad);

				dad = new Character(0, 0, 'fleetway-extras3');
				dad.alpha = 0.01;
				add(dad);
				remove(dad);
		...
		else if (curSong.toLowerCase() == 'triple-trouble' || isRing)
		{
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong, " ", "-").toLowerCase())
			{
				...
				default:
				    if (curSong.toLowerCase() == 'triple-trouble' || isRing) {
					    startCountdown();
					    add(blackFuck);
		...
		else
		{
			if (isStoryMode || isList)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (curSong == 'triple-trouble' || isRing)
					{
						LoadingState.loadAndSwitchState(new VideoState('assets/videos/soundtestcodes.webm', new MainMenuState()));
					}
					else
				...
				else
				{
					...
					else if (curSong == 'you-cant-run')
					{
						...
						LoadingState.loadAndSwitchState(new VideoState('assets/videos/youcantruncutscene2.webm', new PlayState()));
					}
					else
			...
			else
			{
				...
				switch (curSong)
				{
					default:
						if (!isFreeplay)
						    if (SONG.song.toLowerCase() == 'triple-trouble' || isRing) {
							    LoadingState.loadAndSwitchState(new VideoState('assets/videos/soundtestcodes.webm', new MainMenuState()));
							} else {
					...
					case 'too-slow':
						LoadingState.loadAndSwitchState(new VideoState('assets/videos/tooslowcutscene2.webm', new MainMenuState()));
					case 'you-cant-run':
						LoadingState.loadAndSwitchState(new VideoState('assets/videos/youcantruncutscene2.webm', new MainMenuState()));
				}
			}
		}
	}
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		var pressArray:Array<Bool> = [_hitbox.K1.justPressed, _hitbox.K2.justPressed, _hitbox.K3.justPressed, _hitbox.K4.justPressed];
		var releaseArray:Array<Bool> = [_hitbox.K1.justReleased, _hitbox.K2.justReleased, _hitbox.K3.justReleased, _hitbox.K4.justReleased];
		if (isRing)
		{
			holdArray = [_hitbox.K1.pressed, _hitbox.K2.pressed, _hitbox.KSP.pressed, _hitbox.K3.pressed, _hitbox.K4.pressed];
			pressArray = [
				_hitbox.K1.justPressed,
				_hitbox.K2.justPressed,
				_hitbox.KSP.justPressed,
	...
	public function backgroundVideo(source:String) // for background videos
	{
		...
		GlobalVideo.getWebm().webm.SKIP_STEP_LIMIT = 90;
